...
class TelloDriverNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('tello_video_port', 11111)
        self.declare_parameter('tello_state_port', 8890)


        self.tello = Tello(str(self.get_parameter('tello_ip').value))
        self.tello.connect()
        self.tello.change_vs_udp(int(self.get_parameter('tello_state_port').value), int(self.get_parameter('tello_video_port').value))
        self.tello.streamon()
    ...
    def cb_command_(self, goal_handle):
        ...
        if (goal_handle.request.command == "wifi"):
            self.tello.connect_to_wifi("iPhone", "helloworld")
            goal_handle.succeed()
